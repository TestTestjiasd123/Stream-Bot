...
const Streamer = dvs.Streamer;
const VideoStream = dvs.VideoStream;
const AudioStream = dvs.AudioStream;
...
console.log('[DEBUG] Streamer:', Streamer);
console.log('[DEBUG] VideoStream:', VideoStream);
console.log('[DEBUG] AudioStream:', AudioStream);
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        resolve({ title: info.title || 'Unknown', duration: info.duration || 0, isLive: info.is_live || false });
      } catch (e) {
